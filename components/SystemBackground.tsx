...
export const SystemBackground: React.FC = () => {
  return (
    <div className="fixed inset-0 z-[-1] bg-neon-dark overflow-hidden pointer-events-none select-none">
      ...
      <div className="absolute top-1/3 left-4 md:left-10 font-mono text-[8px] md:text-[10px] text-neon-blue/20 flex flex-col gap-2">
         {[...Array(12)].map((_, i) => (
             <motion.div key={`l-${i}`} 
      ...
      <div className="absolute bottom-1/3 right-4 md:right-10 font-mono text-[8px] md:text-[10px] text-neon-blue/20 flex flex-col gap-2 text-right">
         {[...Array(12)].map((_, i) => (
             <motion.div key={`r-${i}`} 
                animate={{ opacity: [0.1, 0.5, 0.1] }}
                transition={{ duration: 2 + Math.random(), repeat: Infinity, delay: Math.random() }}
